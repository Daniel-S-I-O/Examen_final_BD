import {
  ...
  const listarcalificacionesPorId = async (req, res) => { 
    try {
      //  Ejecutar la consulta en la base de datos
      const calificaciones = await listarcalificacionesPorIdQuery(req.params.id_calificaciones);
      res.json(calificaciones);
    } catch (error) {
      res.status(500).send(error);
    }
  ...
  const crearcalificaciones = async (req, res) => {
    ...
    try {
        ...
        res.json({ mensaje: 'calificaciones creado con éxito', id_calificaciones: resultado.insertId });
    } catch (error) {
        res.status(500).send(error);
    }
  };
